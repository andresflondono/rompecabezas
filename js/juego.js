...
var instrucciones = ["1. Inicia armando el rompecabezas por los bordes y sus esquinas.", "2. Si estas en problemas, refresca la pantalla para volver a iniciar", "Al lograr armarlo, te avisaremos."];
...
Podés ver su implementación en la ultima parte de este codigo. //COMPLETAR */
...
var ultimaDireccion = function(direccion) {

    movimientos.push(direccion);
    //console.log(movimientos); // verifique que la variable movimientos, me guarda todos los movimientos del juego.
    // podria mostrarlos todos en el html modificando la funcion actualizarUltimoMovimiento, podria mostrar todo el array movimientos en el html al final del juego.



    actualizarUltimoMovimiento(direccion); // muestra el ultimo movimiento en el html. Como seria mostrar todos?
...
function chequearSiGano() {
    var contador = 0; // debo ponerlo para verificar que el llegue a 9 para saber que cumplio todo y luego mostrar cartel.
    for (i = 0; i < grilla.length; i++) {
        for (j = 0; j < grilla[i].length; j++) {
            var valorActual = grilla[i][j];
            if (valorActual == grillaCorrecta[i][j]) {
                contador++;
            } else {
                return false;
            }
...
function mostrarCartelGanador() {
    if (chequearSiGano() === true) {
        alert("Felicitaciones lo has logrado");
        mezclarPiezas(60); // mezclo de nuevo 
        movimientos = []; // movimientos de nuevo en cero.
...
function posicionValida(fila, columna) {
    //COMPLETAR, LA GRILLA ES 3X3 EN INDICES O POSICIONES SERIA NUMERO ENTRE 0 Y 2, CADA ARREGLO TIENE 3 POSICIONES PERO INICIA EN POS0, ES DECIR, POSO, POS1 Y POS2, FILAS Y COLIMNAS.
...
/* Movimiento de fichas, en este caso la que se mueve es la roja intercambiando su posición con otro elemento.
Las direcciones están dadas por números que representa: arriba (38), abajo (40), izquierda (37), derecha (39) */
function moverEnDireccion(direccion) {
    var nuevaFilaPiezaVacia;
    var nuevaColumnaPiezaVacia;

    // Mueve pieza hacia la abajo, reemplazandola con la roja
    ...
    // Mueve pieza hacia arriba, reemplazandola con la roja
    ...
    // Mueve pieza hacia la derecha, reemplazandola con la roja
    ...
    // Mueve pieza hacia la izquierda, reemplazandola con la roja
